...
Object::Parallelogram::Parallelogram(const Vector &origin, const Vector &a, const Vector &b, const Material &material) :
        Object(origin, material), a_(a), b_(b) {
    origin_ = origin;
    oa_ = a_ - origin_;
    ob_ = b_ - origin_;
...
Vector Object::Parallelogram::Normal(const Vector &point) { return n_; }
//This does actually not need a point, have to think about this...

double Object::Parallelogram::GetIntersectionDistance(const Ray &ray) {
    Vector dir = ray.GetDirection();
    double det = dir * (oa_ob_);
    //std::cout << "oa: " << oa_ << "ob: " << ob_ << "oa_ob: " << oa_ob_ << std::endl;
    //std::cout << origin_ << std::endl;
